...
class TransferBase(object):
	def __init__(self):
		...
		self.key_list = ['port', 'u', 'd', 'transfer_enable', 'passwd', 'enable', 'level']
	...
	def del_server_out_of_bound_safe(self, last_rows, rows):
		...
		cur_servers = {}
		new_servers = {}
		server = self.get_server_level()
		for row in rows:
			try:
				allow = switchrule.isTurnOn(row) and row['enable'] == 1 and row['level'] > 0 and row['u'] + row['d'] < row['transfer_enable']
			except Exception as e:
				allow = False
...
class DbTransfer(TransferBase):
	...
	def get_server_level(self):
		...
		cur = conn.cursor()
		cur.execute("SELECT * FROM ss_node WHERE `id`='" + str(self.cfg["node_id"]) + "'")
